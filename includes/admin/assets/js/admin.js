...
function Link_emsFormBuilder(state) {
  ...
  switch(state){
    ...
    case  'ws':
      ...
      break;
    case  'efb':
      link = "https://wordpress.org/plugins/SEXTOHafez/";
      break;
    case  'tutorial':
    case  'wiki':
      link = link + 'documents';
     ...
     <div class="container"><div class="row row-cols-1 row-cols-md-2 g-4">${value}<div class="row  my-5 col-2"></div></div></div>
      ...
      for(const n of newform_){
          ...
          n.addEventListener("click", (e) => {
            ...
            create_form_by_type_SEXTOHafez(n.id);
...
function create_form_by_type_SEXTOHafez(id){
  //v2
  const state =false;
...
function head_introduce_efb(state){
  //v2
  const link = state=="create" ? '#form' : 'admin.php?page=SEXTOHafez_create'
