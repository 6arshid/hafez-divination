...
class _Public {
	...
	public function EMS_Form_Builder($id){
		...
		$this->id = $id;
		if($this->value==null){
			return "<div id='body_efb' class='card card-public pb-3 efb'> <div class='text-center my-5'><div class='text-danger bi-exclamation-triangle-fill efb text-center display-1 my-2'></div><h3 class='efb text-center text-darkb fs-4'>".__('Form does not exist !!','SEXTOHafez')."</h3><h6 class='text-center my-1 text-pinkEfb efb'>".__('Easy Form Builder', 'SEXTOHafez')."</h6></div></div>";
		}
		$lang = get_locale();
		if ( strlen( $lang ) > 0 ) {
		...
		if(gettype($stng)=="integer" && $stng==0){
			$stng=__("Settings not found",'SEXTOHafez');
			$state="form";
		...
		array( 'ajax_url' => admin_url( 'admin-ajax.php' ),			
		 ...
		 $k="";
		// $pro=false;
		 if(gettype($stng)!=="integer" && $stng!= __("Settings not found",'SEXTOHafez')){
		 ...
		 $efb_m ="<h6 class='text-center my-1 text-pinkEfb efb'>".__('Easy Form Builder', 'SEXTOHafez')."</h6> ";
	
		 if($pro==true) $efb_m="";
		 if($formObj[0]["stateForm"]==true){
			$content ="<div id='body_efb' class='card card-public pb-3 efb'> <div class='text-center my-5'>
			<div class='bi-shield-lock-fill efb text-center display-1 my-2'></div><h3 class='efb text-center text-darkb fs-5'>".__('Form is Private, Please Login.','SEXTOHafez')."</h3>
			 ".$efb_m."
			</div> </div>";
		 }else{
			 ...
			 <div class='lds-hourglass efb text-center my-2'></div><h3 class='efb text-center text-darkb fs-5'>".__('Please Waiting','SEXTOHafez')."</h2>
			 ".$efb_m."
			 </div>
	...
	function public_scripts_and_css_head(){
	  ...
	  public function get_ajax_form_public(){
		...
		if (check_ajax_referer('public-nonce','nonce')!=1){
			...
			$response = array( 'success' => false  , 'm'=>__('Security Error 403', 'SEXTOHafez')); 
			...
			//error_log($setting->activeCode);
				...
				if($formObj[0]['captcha']==true){
					if(strlen($secretKey)>5){
						...
						$response = array( 'success' => false  , 'm'=>__('Error, Check site Key and secret Key on Easy Form Builder > Panel > Setting > Google Keys' , 'SEXTOHafez')); 
					}
				}
			}
			if ($type=="logout" || $type=="recovery") {$not_captcha==true;}
		if ($not_captcha==false && $captcha_success->success==false  ) {
		  $response = array( 'success' => false  , 'm'=>__('Error, Captcha problem!' , 'SEXTOHafez')); 
		  wp_send_json_success($response,$_POST);
		  die();
		}else if ( $not_captcha==true || $captcha_success->success==true) {
			if(empty($_POST['value']) || empty($_POST['name']) || empty($_POST['id']) ){
				$response = array( 'success' => false , "m"=>__("Please enter a valid value" , 'SEXTOHafez')); 
				wp_send_json_success($response,$_POST);
				die();
			}
				...
				foreach($valobj as $key => $val){	
					...
					switch($type){
						...
						case "register":
							...
							$m =__('Your account has been successfully created! You will receive an email containing your information','SEXTOHafez');
						...
						case "login":
							...
							if(isset($user->ID)){
								...
								$send['state']=false;
								$send['pro']=$pro;
								$send['error']=__('The username or password is incorrect' , 'SEXTOHafez');
								$response = array( 'success' => true , 'm' =>$send); 
								wp_send_json_success($response,$_POST);
							}
						...
						case "recovery":
							...
							if($email!="null"){
								...
								if(gettype($state)=="object"){
									...
									if($pro==false) $efb ='<p> '. __("from").''. home_url(). ' '. __("sent by:" , 'SEXTOHafez') .'<b>['. __('Easy Form Builder' , 'SEXTOHafez') .']</b></p>' ;
							...
							$m=__('If your email is correct, the new password will send to your email.', 'SEXTOHafez');
							$response = array( 'success' => true , 'm' =>$m); 
							wp_send_json_success($response,$_POST);
						break;
						case "subscribe":
							...
							$response = array( 'success' => true , 'm' =>__('Done', 'SEXTOHafez')); 
							wp_send_json_success($response,$_POST);
						break;
						case "survey":
							$this->get_ip_address();
							...
							$response = array( 'success' => true , 'm' =>__('survey has been completed', 'SEXTOHafez')); 
							wp_send_json_success($response,$_POST);
						break;
						case "reservation":
						break;
						...
						default:
						$response = array( 'success' => false  ,'m'=>__('Security Error 405', 'SEXTOHafez')); 
						wp_send_json_success($response,$_POST);
					}
		...
		$response = array( 'success' => false , "m"=>__("Error,Setting not Found" , 'SEXTOHafez')); 
		wp_send_json_success($response,$_POST);
	}
	  ...
	  public function get_ajax_track_public(){
		...
		if(gettype($r)=="object"){
		 ...
		 // "Error, you are a robot?";
		  $response = array( 'success' => false  , 'm'=>__('Error, Are you a robot?' , 'SEXTOHafez')); 
		  wp_send_json_success($response,$_POST);
		 }
		...
		if(empty($_POST['value']) ){
			$response = array( 'success' => false , "m"=>__("Please enter vaild values", 'SEXTOHafez')); 
			wp_send_json_success($response,$_POST);
			die();
		}
			...
			if($value!=null){
				...
				$response = array( 'success' => false  , "m" =>__("Confirmation Code not found!", 'SEXTOHafez')); 
			}
	...
	public function file_upload_public(){
		...
		if (in_array($_FILES['file']['type'], $arr_ext)) { 
			...
			$response = array( 'success' => false  ,'error'=>__("File Permissions Error", 'SEXTOHafez')); 
			wp_send_json_success($response,$_POST);
	...
	public function set_rMessage_id_Emsfb(){
		...
		if(empty($_POST['id']) ){
			$response = array( 'success' => false , "m"=>__("Something went wrong ,Please refresh and try again", 'SEXTOHafez')); 
			wp_send_json_success($response,$_POST);
			die();
		}
		...
		if($this->isHTML($_POST['message'])){
			$response = array( 'success' => false , "m"=> __("You are not allowed use HTML tag", 'SEXTOHafez')); 
			wp_send_json_success($response,$_POST);
			die();
		}
		...
		if(gettype($r)=="object"){
			...
			$id;
				...
				$response = array( 'success' => true , "m"=>__("Message was sent" , 'SEXTOHafez') , "by"=>$by); 										
				wp_send_json_success($response,$_POST);	
		}else{
			$response = array( 'success' => false , "m"=>__("Message was not sent! <br> Please Contact to admin, Settings Error" , 'SEXTOHafez') , "by"=>$by);
			wp_send_json_success($response,$_POST);	
		}
   ...
   if($state=="notiToUserFormFilled_TrackingCode"){
	$subject =__('We received your Message', 'SEXTOHafez');
	$message ="<h2>".__('Thank You for filling out the form', 'SEXTOHafez')."</h2>
			<p>". __('Confirmation Code' , 'SEXTOHafez').": ".$cont." </p>
	...
	$subject =__('Thank You for registering.', 'SEXTOHafez');   
	$message ="<h2>".__('Welcome', 'SEXTOHafez')."</h2>
	".$cont."
	<button><a href='".home_url()."' style='color: white;'>".get_bloginfo('name')."</a></button>
	";
	$cont=$message;
   }elseif ($state=="subscribe"){
	$subject =__('Welcome', 'SEXTOHafez');   
	$message ="<h2>".__('Thank You For Subscribing!', 'SEXTOHafez')."</h2>
	<button><a href='".home_url()."' style='color: white;'>".get_bloginfo('name')."</a></button>
	";
	$cont=$message;
   }elseif ($state=="survey"){
	$subject =__('Welcome', 'SEXTOHafez');   
	$message ="<h2>".__('Thank You for taking the time to complete this survey.', 'SEXTOHafez')."</h2>
	<button><a href='".home_url()."' style='color: white;'>".get_bloginfo('name')."</a></button>
	";
	$cont=$message;
   }
